...
public class CommandPay implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender cs, Command command, String label, String[] args) {
        Bukkit.getScheduler().runTaskAsynchronously(CityBuildStuff.getInstance(), () -> {
            ...
            CoinApi coinAPISender = new CoinApi(((Player) cs).getUniqueId());

            if (coinAPISender.getCoins() < coins) {
                cs.sendMessage((ChatColor.translateAlternateColorCodes('&',
            ...
            cs.sendMessage((ChatColor.translateAlternateColorCodes('&',
                            ...
                            CityBuildStuff.getInstance().getConfig().getString("Coins.prefix"))) + " " + "an §2" + target.getName() + " §7überwiesen!");

            CoinApi coinAPITarget = new CoinApi(target.getUniqueId());
            ...
            target.sendMessage((ChatColor.translateAlternateColorCodes('&',
                            ...
                            CityBuildStuff.getInstance().getConfig().getString("Coins.prefix"))) + "" + " überwiesen!");

            if (target != cs) {
                coinAPISender.removeCoins(coins);
                coinAPITarget.addCoins(coins);
            }
        });
        return true;
    }
